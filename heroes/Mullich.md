...
### 3 Spell: Fast Attack
 **Description:** Increases ATK speed by <span style="color: 48b946">]{($buffaddattr12+$buffaddattr13*($sklevel-1))}</span><($buffaddattr12+$buffaddattr13*($sklevel-1))*($a124+$a126+$a134+$a136)><span style="color: 48b946">]%</span> and movement speed by {$buffaddattr22+$buffaddattr23*($sklevel-1)}<($buffaddattr22+$buffaddattr23*($sklevel-1))*($a124+$a126+$a134+$a136)> for friendly units in a large range for {($bufflast11+$bufflast12*($sklevel-1))/1000}<($bufflast11+$bufflast12*($sklevel-1))/1000*$talent4+$talent3>s.

### 4 Spell: Time Stop
 **Description:** Summons a large range of Time Field, immobilizing enemy units in range for <span style="color: 48b946">]{($olast11+$olast12*($sklevel-1))/1000}</span>s.
