...
### 4 Spell: Break Waves
 **Description:** Peter Lee drives an assault ship into enemy units, dealing <span style="color: #48b946">{($valueadd11+$valueadd12*($sklevel-1))}<span style="color: black"><($valueadd11+$valueadd12*($sklevel-1))*($a123+$a126+$a128+$a131)+$a101+(($valueadd11+$valueadd12*($sklevel-1))+($valueadd11+$valueadd12*($sklevel-1))*($a123+$a126+$a128+$a131)+$a101)*$talent2+$talent3> water spell damage on all enemy units. Defensive units take additional spell damage equal to 10% of their Max HP.
 ...
 **Description:** Nothing is faster on the seas than my raven. Even a storm can't keep up with it!
